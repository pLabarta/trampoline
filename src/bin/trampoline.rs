use anyhow::Result;
use ckb_types::{H256, h256, h160, H160};
...
use ckb_app_config::{AppConfig, CKBAppConfig, BlockAssemblerConfig};
use ckb_system_scripts::*;
use trampoline::opts::{TrampolineCommand, SchemaCommand, NetworkCommands};
use trampoline::{TEMPLATES, TrampolineResourceType};
use trampoline::project::*;
use trampoline::docker::*;
use trampoline::TrampolineResource;
use trampoline::schema::{Schema, SchemaInitArgs, SchemaError};
use trampoline::{hex_string, hex_decode, hex_encode, parse_hex};
use ckb_hash::{blake2b_256};
use anyhow::anyhow;
use ckb_types::prelude::Pack;

const SECP_TYPE_HASH: H256 = h256!("0x9bd7e06f3ecf4be0f2fcd2188b23f1b9fcc88e5d4b65a8637b17723bbda3cce8");
        ...
        TrampolineCommand::Schema { command } => {
            match command {
                SchemaCommand::New {name, def} => {
                    let mut schema_args: SchemaInitArgs = (TrampolineProject::from(project?), name, None);
                    if def.is_some() {
        ...
        TrampolineCommand::Network {command} => {
            ...
            match command {
                ...
                NetworkCommands::Launch {} => {
                    ...
                    let miner_ports = (miner_config.host_port.into(), miner_config.container_port.into());
                    Docker::default()
                        .name(project.config.name.as_str())
                        .add_service(project.config.env.unwrap().chain)?
                        .run(Some(vec!["run".to_string()]), Some("node"), vec![miner_ports])?;
                },
                ...
                NetworkCommands::Miner {one_block: _} => {
                    ...
                    Docker::exec(format!("{}-node", container_name).as_str(),
                     vec!["ckb", "miner"],
                     miner_mount_path
                    )?;
                }
